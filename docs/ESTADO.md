...
**v3 hecha: frontend ACP por stdio (`--acp`).** Antes, v2 puntos 1 y 2 (tool `task` y subagentes, skills) más la
tool `fetch`, ya mergeados a `main` en los PR #3 y #4. De v2 solo queda el proveedor `copilot`, que se salta hasta
tener acceso para probarlo; por eso se adelantó la v3.
...
1. **Abrir kotycli desde Emacs con agent-shell** (el criterio de éxito de la v3 en el roadmap) y arreglar lo que
   ...
   cliente propio, pero eso no es lo mismo que contra uno ajeno. Con Zed y Neovim, igual.
2. **Proveedor `copilot`** (v2 punto 3, `06-proveedores.md`, ADR 0012): `OpenAiProvider` con un `Authenticator`
...
3. **Comandos `/` de la TUI**, lo que queda de v1.1: `/compact` (implica escribir la compactación en
   `ContextManager`, nivel 3), `/config`, `/copy`, `/edit` al `$EDITOR`, `/reload`. El completer de JLine ya
   está puesto, con los skills dentro. `/compact` tendría que emitir `Compacted`, que ACP y la TUI ya pintan.
...
```
git fetch origin && git checkout claude/v3-continue-previous-state-fvn8gm
./gradlew build
```
